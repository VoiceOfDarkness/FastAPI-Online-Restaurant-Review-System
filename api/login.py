...
from utils.auth_session import get_current_user, secret_key
...
@login_router.post("/login/authenticate", tags=['User'])
async def authenticate(
    ...
    password: str = Query(
        ..., description="The password of the credentials", max_length=20
    ),
...
@login_router.post("/login/add", tags=['User'])
async def add_login(
    req: LoginReq,
    engine=Depends(create_db_engine),
    ...
    if result == True:
        ...
        return JSONResponse(
            content={"message": "insert login unsuccessful"},
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
        )


@login_router.post("/login/profile/add", tags=['User'])
async def add_login_profile(
    ...
    repo: LoginRepository = LoginRepository(engine)
    result = repo.add_profile(loads(profile_json), username)
    if result:
        return req
    else:
        return JSONResponse(
            content={"message": "Insert purchase unsuccessful"},
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
        )


@login_router.get("/login/list/all", tags=['User'])
async def list_all_login(
    ...
    return ORJSONResponse(
        content=jsonable_encoder(result), status_code=status.HTTP_201_CREATED
    )


@login_router.get("/login/account/{id}", tags=['User'])
async def get_login(
...
@login_router.get("/logout", tags=['User'])
async def logout(
    response: Response,
    engine=Depends(create_db_engine),
    user: str = Depends(get_current_user) 
...
@login_router.get("/signup", tags=['User'])
async def signup(
    engine=Depends(create_db_engine),
    user: str = Depends(get_current_user) 
):
    ...
    """
    return HTMLResponse(content=signup_content, status_code=status.HTTP_200_OK)


@login_router.get('/login/html/list', tags=['User'])
...
@login_router.get('/login/enc/details', tags=['User'])
