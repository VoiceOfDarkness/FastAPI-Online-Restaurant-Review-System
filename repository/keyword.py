...
class KeyRepository:
    ...
    async def insert_keyword(self, details: Dict[str, Any]):
        try:
    ...
    async def update_keyword(self, word: str, details: Dict[str, Any]):
        try:
            keyword = await self.engine.find_one(Keyword, Keyword.word == word)
            
            for key, val in details.items():
                setattr(keyword, key, val)
    ...
    async def delete_keyword(self, word: str):
        try:
            keyword = await self.engine.find_one(Keyword, Keyword.word == word)
