...
coockie_sec = APIKeyCookie(name='session')
...
async def get_current_user(session: str = Depends(coockie_sec), engine=Depends(create_db_engine)):
    try:
...
class SessionDbMiddleware(BaseHTTPMiddleware):
    def __init__(self, app, sess_key: str, sess_name: str, expiry: str):
        ...
        self.engine = AIOEngine(client=self.client_od, database='orrs')
        ...
        try:
            if re.search(r'\bauthenticate\b', request.url.path):
                ...
                if login == None:
                    ...
                    return JSONResponse(content='some problem occurres', status_code=403)
                else:
